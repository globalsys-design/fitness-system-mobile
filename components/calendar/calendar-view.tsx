...
export function CalendarView({ events }: CalendarViewProps) {
  ...
  return (
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* CABEÇALHO FIXO: shrink-0 — grid do calendário nunca faz scroll */}
      <div className="shrink-0 bg-background z-10">
        ...
        {/* Separador + label do dia selecionado */}
        {selectedDay && (
      ...
      {/* SCROLL ISOLADO: eventos do dia selecionado — flex-1 ocupa espaço restante */}
      <div className="flex-1 overflow-y-auto min-h-0 px-4 pb-32 pt-2">
        {!selectedDay ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            <CalendarDays className="size-10 text-muted-foreground" />
            <p className="text-sm text-muted-foreground text-center">
              Toque num dia para ver os agendamentos
            </p>
          </div>
      ...
      <BottomSheet open={eventDetailOpen} onOpenChange={setEventDetailOpen} title={selectedEvent?.title}>
        {selectedEvent && (
      ...
      <FAB icon={Plus} onClick={() => router.push("/app/mais/calendario/novo")} label="Novo Agendamento" />
    </div>
  );
}
