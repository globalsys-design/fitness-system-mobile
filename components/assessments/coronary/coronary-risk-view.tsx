...
export function CoronaryRiskView({
  ...
  /* Contribuição de cada questão (pontos absolutos, com cor por intensidade) */
  const contributions = useMemo(() => {
    return questions.map((q) => {
      const opt = findOption(q.key, answers[q.key]);
      const points = opt?.points ?? 0;
      const max = Math.max(...q.options.map((o) => o.points));
      const intensity = max > 0 ? points / max : 0; // 0..1
      return {
        key: q.key,
        label: q.label,
  ...
  return (
    <div className="relative pb-[calc(7rem+env(safe-area-inset-bottom))]">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <ContributionBar contributions={contributions} maxScore={maxScore} />
...
/* ──────────────────────────────────────────────────────────────────────
 * ContributionBar — barra empilhada mostrando contribuição de cada fator
 * ────────────────────────────────────────────────────────────────────── */
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
          {contributions.length} fatores
        </span>
      </div>

      {/* Barra empilhada */}
      <div className="relative h-3 w-full rounded-full bg-muted/40 overflow-hidden flex">
        {contributions.map((c, i) => {
          // largura: proporção dentro do maxScore total (não da própria max),
          // assim a barra sempre representa o todo (0..maxScore)
          const widthPct = (c.points / maxScore) * 100;
          ...
          return (
            <div
              key={c.key}
              className={cn(tone, "spell-bar-grow h-full")}
              style={
                {
                  width: `${widthPct}%`,
      ...
      {/* Legenda compacta — chips por fator */}
      <div className="flex flex-wrap gap-1.5">
        {contributions.map((c, i) => (
          <span
            key={c.key}
            className={cn(
              "spell-slide-up inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold",
              c.intensity === 0
